...
public class RedisCache extends AbstractValueAdaptingCache {
	...
	public RedisCache(String name, byte[] prefix, RedisOperations<? extends Object, ? extends Object> redisOperations,
		...
		this(name, prefix, redisOperations, redisConfig, false);
	}
	 ...
	 */
	public RedisCache(String name, byte[] prefix, RedisOperations<? extends Object, ? extends Object> redisOperations,
			RedisConfig redisConfig, boolean allowNullValues) {
		...
		this.cacheMetadata.setRedisConfig(redisConfig);
	...
	public ValueWrapper get(Object key) {
		RedisCacheElement element = get(getRedisCacheKey(key));
		if (element != null) {
			/* 这里在获取缓存时判断缓存是否为空 不为空则重新设置一下改缓存的有效期做到延时功能 */
			if (this.cacheMetadata.getRedisConfig().getIsDelay()
					&& this.cacheMetadata.getRedisConfig().getExpire() != 0) {
				redisOperations.expire(key, this.cacheMetadata.getRedisConfig().getExpire(), TimeUnit.SECONDS);
			}
		}
		return element;
	}
	...
	public <T> T get(final Object key, final Callable<T> valueLoader) {
		...
		RedisCacheElement cacheElement = new RedisCacheElement(getRedisCacheKey(key),
				new StoreTranslatingCallable(valueLoader)).expireAfter(cacheMetadata.getRedisConfig().getExpire());
	 ...
	 *
	 * @param cacheKey
	 *            the key whose associated value is to be returned via its binary
	 *            representation.
	 * @return the {@link RedisCacheElement} stored at given key or {@literal null}
	 *         if no value found for key.
	 * @since 1.5
	 */
	...
	public void put(final Object key, final Object value) {
		...
		put(new RedisCacheElement(getRedisCacheKey(key), toStoreValue(value))
				.expireAfter(cacheMetadata.getRedisConfig().getExpire()));
	}
	 ...
	 * 
	 * @see
	 * org.springframework.cache.support.AbstractValueAdaptingCache#fromStoreValue(
	 * java.lang.Object)
	 */
	@Override
	 ...
	 * {@link RedisCacheElement#getKeyBytes()}. If the cache previously contained a
	 * mapping for this {@link RedisCacheElement#getKeyBytes()}, the old value is
	 * replaced by {@link RedisCacheElement#get()}.
	 *
	 * @param element
	...
	/**
	 * {@inheritDoc} This implementation simply returns the RedisTemplate used for
	 * configuring the cache, giving access to the underlying Redis store.
	 */
	...
	/**
	 * {@link Callable} to transform a value obtained from another {@link Callable}
	 * to its store value.
	 * 
	 * @author Mark Paluch
	 * @since 1.8
	...
	/**
	 * Metadata required to maintain {@link RedisCache}. Keeps track of additional
	 * data structures required for processing cache operations.
	 *
	 * @author Christoph Strobl
	 * @since 1.5
	 */
	static class RedisCacheMetadata {
		...
		/**
		 * Get the binary representation of the key identifying the data structure used
		 * to maintain known keys.
		 *
		 * @return never {@literal null}.
		 */
	...
	static class RedisCacheCleanByKeysCallback extends LockingRedisCacheCallback<Void> {
		 ...
		 * 
		 * @see
		 * org.springframework.data.redis.cache.RedisCache.LockingRedisCacheCallback#
		 * doInLock(org.springframework.data.redis.connection.RedisConnection)
		 */
		@Override
	...
	static class RedisCacheCleanByPrefixCallback extends LockingRedisCacheCallback<Void> {
		...
		public Void doInLock(RedisConnection connection) throws DataAccessException {
			...
			if (isClusterConnection(connection)) {
				...
				// load keys to the client because currently Redis Cluster connections do not
				// allow eval of lua scripts.
	...
	static class RedisCacheEvictCallback extends AbstractRedisCacheCallback<Void> {
		 ...
		 * 
		 * @see
		 * org.springframework.data.redis.cache.RedisCache.AbstractRedisCacheCallback#
		 * doInRedis(org.springframework.data.redis.cache.RedisCacheElement,
	...
	static class RedisCachePutCallback extends AbstractRedisCacheCallback<Void> {
		 ...
		 * 
		 * @see
		 * org.springframework.data.redis.cache.RedisCache.AbstractRedisPutCallback#
		 * doInRedis(org.springframework.data.redis.cache.RedisCache.RedisCacheElement,
		 * org.springframework.data.redis.connection.RedisConnection)
		 */
		@Override
		public Void doInRedis(BinaryRedisCacheElement element, RedisConnection connection) throws DataAccessException {
